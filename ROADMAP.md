...
- **브라우저로 하는 일은 두 모드뿐 (2026-07-30 Dan 확정).** ① **자동 처리(A)** — 공식 기계 경로(Export/Import·API)가 있거나 조작이 결정적일 때. 사람은 버튼 하나 누르고 기다린다(예: 데이터 안전 CSV 왕복). ② **가이드(B, 코파일럿)** — 기계 경로가 없거나 판단이 사람 몫·비가역일 때. **진짜 콘솔 폼 옆에 AI를 붙여** 사람이 한 번만 조작하게 한다(예: 콘텐츠 등급·타깃 연령). A가 막히면 그 자리에서 B로 흐른다(핸드오프) — 둘은 대립이 아니라 같은 축의 양 끝이다. **금지: 넣어줄 수 없는 폼을 ZTO 안에 복제하는 것** — 그건 도움이 아니라 입력을 두 번 시키는 것이다. 판별식은 "사용자가 여기 답하면 우리가 반영할 수 있나?". 상세는 `docs/BROWSER-AUTOMATION.md` §0-1
...
- [x] **codex(ChatGPT 구독) 경로 (2026-07-29)** — `codex exec --json`의 JSONL 이벤트를 읽는다(`thread.started.thread_id`=세션 id, `agent_message` 아이템=최종 답변, `turn.completed.usage`=토큰). 이어가기는 `codex exec resume <id> --json`. **모델은 지정하지 않는다** — `--model` 플래그가 문서에서 확인되지 않았고 구독 모델 id는 API 목록(gpt-5.4-*)과 다르므로, 구독 모드에선 모델 선택지를 `Codex` 하나로 두고 codex 자체 기본값을 쓴다(고를 수 없는 걸 고르게 하면 거짓말). **이미지는 명시적으로 거절**(`codex exec`에 문서화된 방법 없음) — 조용히 버리면 "AI가 화면을 못 봤다"는 사실이 감춰진다. ⚠️ **실행 미검증**: 개발 기기에 codex 미설치·구독 없음. 그래서 파서를 관대하게 쓰고 실패 시 stderr를 그대로 올린다 — 추측이 틀렸을 때 빈 답이 아니라 원인이 보이도록. **구독 보유자에게서 첫 실행 검증 필요**
  - 참고(비용 판단, Dan 2026-07-29): ZTO 자신의 사용량만 보면 API 키가 월 $2 수준이라 $20 구독보다 싸고, Codex CLI는 웹·IDE와 5시간 한도를 공유해 코딩 세션이 한도를 태우면 ZTO가 막힌다. 다만 **ZTO는 Dan 전용 도구가 아니므로** 구독 보유 사용자를 위해 경로 자체는 갖춘다
- [x] **AI 사용량 대시보드 (2026-07-29)** — 설정에 모델별 사용량·비용. 호출 1건씩 `userData/zto-ai-usage.json`에 append(로컬 전용, 상한 2000). claude CLI는 응답에 `total_cost_usd`·`usage`를 실어 주므로 그대로 기록하고, OpenAI는 토큰만 주므로 가격표로 환산. **구독 비용은 실지출이 아니다** — `billed` 플래그로 갈라 타일·행에서 분리 표시(합치면 없는 지출을 있는 것처럼 보여준다). 차트는 dataviz 규격 — 검증된 카테고리 팔레트를 **모델 순서 고정** 배정(사용량 순으로 칠하면 필터 시 색이 바뀐다), 막대 높이는 비용이 아니라 **호출 수**(환산가와 실지출을 한 막대에 쌓으면 무의미한 높이가 된다)
...
- [ ] 폼 채우기 — 설문 답을 콘솔 폼에 입력(CDP 합성 입력으로 사이트 검증 통과). 비가역 제출은 사람 컨펌
  ...
  - **곁다리로 열린 것**: 백로그의 *대시보드 설정 항목 자동 점등(L2 콘솔 읽기)* 이 이제 가능하다 — Play에 read API가 없어 "콘솔에서 확인하세요"로 처리하던 선언들이 실제로 읽힌다(위 컨트롤 수 참조). **편집이 아니라 점등**이므로 콘솔을 건드리지 않는다. 자산 편집(#3) 작업 중 곁다리로 붙인다
  - **데이터 안전은 CSV 왕복이 정답** — 구글이 Export/Import CSV를 공식 제공한다(217문항이 기계 판독 ID로). 쓰기도 CDP `DOM.setFileInputFiles`로 파일만 꽂으면 되므로 5단계 위저드를 클릭으로 따라갈 필요가 없다. DOM 스크래핑은 CSV가 없는 설문(콘텐츠 등급·타깃 연령)에만
  - **작업 방식은 `docs/BROWSER-AUTOMATION.md`에 이식 가능한 형태로 정리** (2026-07-30) — 경로 조립 금지·도착 검증·고정대기 금지·숨은 뷰 스로틀·핸드오프 등 실패로 얻은 14개 규칙. 다른 프로젝트(libertas-marketing-plugins 등)에 그대로 건넬 수 있다
...
- [x] **팝업 로그인 (2026-07-29 해결·실증)** — 어제 "미검증"으로 남긴 구멍이 실제로 터졌다: Instagram/Threads의 "Google로 로그인"이 흰 화면으로 굳음. 원인은 `setWindowOpenHandler`가 팝업 URL을 **같은 탭에 로드**(deny)한 것 — 구글 GSI(`accounts.google.com/gsi/select`)는 팝업 전용 페이지라 `window.opener.postMessage`로 부모에 결과를 돌려주는데, 같은 탭에 로드하면 ① opener가 없고 ② 결과를 받을 원래 페이지도 navigate돼 사라진다. **어제 Play 콘솔·ASC가 통과한 건 그쪽이 전체 페이지 리다이렉트라 opener가 필요 없어서였다** — 같은 '로그인'이라도 방식이 달라 한쪽만 통과했던 것. 해결: `action: 'allow'`로 **진짜 팝업 창**(520×680, 메인 창에 parent로 물림, http/https만 허용). 기본 세션 공유라 팝업에서 한 로그인이 탭에도 반영된다(A안 유지). Dan 실기 확인 완료
- [x] **탭 순서 드래그 변경 + ⌘N 뱃지 (2026-07-29)** — `browser:moveTab`이 main의 `tabs` **배열 자체를 옮긴다**. 단축키가 인덱스 기준(`selectIndex`)이라 렌더러에서만 재정렬하면 화면 순서와 ⌘N이 어긋난다. 탭마다 `⌘1..9` 뱃지를 달아 매핑을 눈에 보이게 함. 활성 탭만 창에 붙으므로(`showActive`) 배열 재정렬은 렌더링에 영향 없음
- [ ] 다중 창 대응, 뷰/탭 상태 영속
